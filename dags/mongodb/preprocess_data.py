from airflow.providers.standard.operators.trigger_dagrun import TriggerDagRunOperator
from airflow.providers.standard.operators.python import PythonOperator
from airflow.sdk import timezone
from airflow import DAG

from plugins.utils import get_raw_data, preprocess_raw_data, save_mongodb_raw_data

with DAG(
    dag_id="coc_mongodb_preprocess_data",
    start_date=timezone.datetime(2025, 12, 9, 0, 0, 0),
    schedule="0 */6 * * *",
    catchup=False,
) as dag:

    get_raw_data_task = PythonOperator(
        task_id="get_raw_data",
        python_callable=get_raw_data
    )

    preprocess_raw_data_task = PythonOperator(
        task_id="preprocess_raw_data",
        python_callable=preprocess_raw_data
    )

    save_mongodb_raw_data_task = PythonOperator(
        task_id="save_mongodb_raw_data",
        python_callable=save_mongodb_raw_data
    )

    trigger_mongodb_postprocess_data_DAG_task = TriggerDagRunOperator(
        task_id='trigger_mongodb_postprocess_data_DAG',
        trigger_dag_id='coc_mongodb_postprocess_data',
        wait_for_completion=False
    )

    get_raw_data_task >> preprocess_raw_data_task >> \
    save_mongodb_raw_data_task >> trigger_mongodb_postprocess_data_DAG_task